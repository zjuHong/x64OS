...
unsigned long sys_open(char *filename, int flags)
{
	...
	else if (pathlen >= PAGE_4K_SIZE) //字符串过长
	{
		kfree(path);
		return -ENAMETOOLONG;
	}
...
unsigned long sys_close(int fd)
{
	...
	kfree(filp);
	current->file_struct[fd] = NULL; //处理文件描述符
...
/** 
 * @brief 系统fork、vfork函数
 * @param 
 * @return 
 */
unsigned long sys_fork()
{
	struct pt_regs *regs = (struct pt_regs *)current->thread->rsp0 - 1; //索引父进程的执行现场
...
unsigned long sys_vfork()
{
	...
	return do_fork(regs, CLONE_VM | CLONE_FS | CLONE_SIGNAL, regs->rsp, 0); //共享资源
}
/** 
 * @brief 系统brk函数
 * @param 
