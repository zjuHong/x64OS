...
long FAT32_write(struct file *filp, char *buf, unsigned long count, long *position)
{
	...
	{//数据写入
		...
		*position += length;
		...
		if (next_cluster >= 0x0ffffff8)//需要扩充容量
		{
			...
			DISK1_FAT32_write_FAT_Entry(fsbi, cluster, next_cluster);//分配新簇
...
struct dir_entry *FAT32_lookup(struct index_node *parent_inode, struct dir_entry *dest_dentry)
{
	...
	cluster = finode->first_cluster; //起始簇号
...
next_cluster:
	...
	cluster = DISK1_FAT32_read_FAT_Entry(fsbi, cluster);//取得父目录的下一个簇号，继续搜索，直至最后一个
...
struct super_block *fat32_read_superblock(struct Disk_Partition_Table_Entry *DPTE, void *buf)
{
	...
	sbp->root->dir_inode->inode_ops = &FAT32_inode_ops;//inode操作方法
	sbp->root->dir_inode->f_ops = &FAT32_file_ops;//file操作方法
